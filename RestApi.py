...
engine = create_engine('mysql+mysqldb://sql7269522:HQXEWyTunX@sql7.freemysqlhosting.net/sql7269522')
...
def post_place():
    ...
    session.add(
        place(Name=data["Name"], Latitude=data["Latitude"], Longitude=data["Longitude"], Country=data["Country"]))
    session.commit()
...
def post_forecast():
    ...
    session.add(forecast(PlaceId=data["PlaceId"], Weather_TypeId=wtId, Wind_DirId=data["Wind_DirId"], Date=data["Date"],
                         Temperature=data["Temperature"], Temperature_Max=data["Temperature_Max"],
                         Temperature_Min=data["Temperature_Min"], Cloud_cover=data["Cloud_cover"],
                         Humidity_percent=data["Humidity_percent"], Pressure_mb=data["Pressure_mb"],
                         Wind_speed=data["Wind_speed"], IsForecast=data["IsForecast"]))
    session.commit()
    return json.dumps({'success': True}), 200, {'ContentType': 'application/json'}
